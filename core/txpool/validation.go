...
func ValidateTransaction(tx *types.Transaction, head *types.Header, signer types.Signer, opts *ValidationOptions) error {
	...
	// Hash-account tx types (0x7C, 0x7F) use codes > 7 and overflow the uint8 Accept mask;
	// they are filtered by FilterType() and validated separately, so skip the bitmask check.
	if !types.IsHashTxType(tx.Type()) {
